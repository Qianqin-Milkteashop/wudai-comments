...
function canEdit(item) {
  ...
  // 游客只能编辑自己创建的内容
  return !!item.createdBy && item.createdBy === CLIENT_ID;
}
...
function openPersonModal(mode, node) {
  ...
  // 检查权限
  const hasPermission = mode === 'add' || canEdit(node);
  
  if (mode === 'edit' && !hasPermission) {
    hint.textContent = '你没有权限编辑此人物。只有创建者或管理员可以编辑。';
  ...
  // 只有有权限且不是中心节点才显示删除按钮
  delBtn.style.display = (mode === 'edit' && node && !node.isCenter && hasPermission) ? 'inline-flex' : 'none';
  delBtn.dataset.nodeId = node?.id || '';
  
  // 没有权限时禁用保存按钮
  saveBtn.disabled = mode === 'edit' && !hasPermission;
  saveBtn.dataset.mode = mode;
  saveBtn.dataset.nodeId = node?.id || '';

  // 没有权限时禁用所有输入框
  const inputs = ['personName', 'personPosition', 'personBirthYear', 'personDeathYear', 'personPersonality'];
  inputs.forEach(id => {
    const el = document.getElementById(id);
    if (el) el.disabled = mode === 'edit' && !hasPermission;
  });
...
function renderSidebar(nodeId) {
  ...
  // 检查是否有编辑权限
  const hasPermission = canEdit(node);
  ...
  wrap.innerHTML = `
    ...
    <div class="detail-actions" id="nodeActions">
      ${hasPermission ? '<button class="btn btn-secondary" id="btn编辑Node">编辑</button>' : '<p class="help-text">只有创建者或管理员可以编辑此人物</p>'}
    </div>
  `;

  const editBtn = document.getElementById('btn编辑Node');
  if (editBtn) {
    editBtn.addEventListener('click', () => openPersonModal('edit', node));
  }
