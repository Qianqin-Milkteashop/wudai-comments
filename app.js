...
<body>
  ...
  <div class="floating-actions">
    ...
    <button class="fab fab-success" id="btnExportImage">
      <span class="fab-icon">📷</span><span class="fab-text">导出图片</span>
    </button>
  </div>
  ...
  <!-- 管理员标记（默认隐藏；按 Esc 三次可打开/退出管理员） -->
  <div id="adminBadge" class="admin-badge hidden">管理员模式</div>
  ...
  <!-- Admin Modal（按 Esc 三次打开；在里面点“退出”即可退出） -->
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <p class="help-text" style="margin-top:0;">
          说明：管理员密钥不会写在网页里，只会保存在你当前浏览器里。
        </p>
        <div class="form-group">
          <label>管理员密钥</label>
          <input id="adminKeyInput" class="input" type="password" placeholder="请输入密钥" autocomplete="off" />
        </div>
        <p class="help-text" id="adminModalHint"></p>
      </div>
      <div class="modal-footer">
        <button class="btn btn-secondary" data-close="adminModal">取消</button>
        <button class="btn btn-danger" id="btnAdminLogout" style="display:none;">退出管理员</button>
        <button class="btn btn-primary" id="btnAdminLogin">进入</button>
      </div>
    </div>
  </div>
